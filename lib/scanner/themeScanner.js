...
export function scanSnippetFilenames(snippetFilenames, activeAppHandles = []) {
  ...
  for (const filename of snippetFilenames) {
    const cleanFilename = filename.toLowerCase().replace(/^snippets\//, '').replace(/\.liquid$/, '');

    for (const app of signaturesData.apps) {
      const matches = app.snippetPatterns.some(pattern => {
        const cleanPattern = pattern.toLowerCase().replace(/^snippets\//, '').replace(/\.liquid$/, '');
        if (cleanFilename === cleanPattern) return true;
        // Delimited prefix or boundary match (e.g. "ssw-helpers" matches "ssw", but "password-layout" does not)
        const regex = new RegExp(`(^|[-_])${cleanPattern.replace(/[-_]$/, '')}([-_]|$)`, 'i');
