 ...
 */
export async function fetchStorefrontHtml(storeUrl, timeoutMs = 15000) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      const response = await fetch(targetUrl, {
        ...
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36 BloatBuster/1.2',
      ...
      if (err.name === 'AbortError') {
        throw new Error(`Storefront request timed out after ${timeoutMs / 1000}s. The store may be slow or temporarily unreachable.`);
      }
...
export function isShopifyPlatformScript(content) {
  if (!content) return false;
  return (
    content.includes('Shopify') ||
    content.includes('__st=') ||
    content.includes('wpmLoader') ||
    content.includes('trekkie') ||
    content.includes('customDocumentWrite') ||
    content.includes('data-nocaptcha') ||
    content.includes('recaptcha') ||
    content.includes('shopify-accelerated') ||
    content.includes('portableWalletsHideBuyerConsent') ||
    content.includes('loader.init-shop-cart-sync') ||
    content.includes('@theme') ||
    content.includes('supportsNativePopover') ||
    content.includes('shopify.content_for_header')
  );
}
...
export function extractResourceTags(html) {
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    if (srcMatch) {
      ...
      // Exclude core Shopify internal inline platform scripts
      if (!isShopifyPlatformScript(inlineContent)) {
        inlineScripts.push({
...
export function scanStorefrontHtml(html, activeAppHandles = []) {
  ...
  for (const script of scripts) {
    ...
    const isShopify = 
      SHOPIFY_SAFE_DOMAINS.some(d => srcLower.includes(d)) ||
      srcLower.startsWith('/cdn/') ||
      srcLower.startsWith('/checkouts/') ||
      srcLower.includes('/cdn/shop/t/') ||
      srcLower.includes('/cdn/shopifycloud/');
